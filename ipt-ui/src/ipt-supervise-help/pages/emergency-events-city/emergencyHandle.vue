<template>
  <div>
    <ol class="breadcrumb">
      <li><a>监管辅助系统</a></li>
      <li><a>跨城市应急事件管理</a></li>
      <li class="active jsh-active">跨城市应急事件处理</li>
    </ol>
    <div class="main">
    <!-- [搜索框] -->
...
<script>
  ...
  import IptCreate from "@/ipt-supervise-help/components/emergency-events-city/emergencyHandleAdd"
  import IptEdit from "@/ipt-supervise-help/components/emergency-events-city/emergencyHandleEdit.vue"
  ...
  export default {
    ...
    methods: {
      ...
      getOne(columnData) {
        Api.EmergencyCityHandle.getOne(columnData.crossCityEmergencyId).then((data) => {
      ...
      approvalBtn(columnData) {
        this.$confirm('是否审核通过?', '提示', {
          ...
          Api.EmergencyCityHandle.approval(columnData.crossCityEmergencyId).then((data) => {
            this.$message({
              type: 'success',
              message: '审核成功!'
            })
      ...
      deleteBtn(columnData) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          Api.EmergencyCityHandle.deleteOne(columnData.crossCityEmergencyId).then((data) => {
