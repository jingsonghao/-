  ...
  export default {
    ...
    watch: {  //弹出层带父页面的值为查询条件时调用 当showIndexData的值发生变化启用
      showIndexData(curVal){
        let obj = this
        obj.reset()
        obj.params.nodeTypeNo = curVal.nodeTypeNo
        obj.params.examineDate = curVal.examineDate
        obj.init()
      }
    },
    computed: mapGetters({
      nodeTypeShowIndex: 'nodeTypeShowIndex',
    ...
    methods: {
      init() {
        ...
        obj.getNodeTypeShowIndex(this.params)
      },
      search() {
        this.getNodeTypeShowIndex(this.params)
      },
      reset() {
        Object.assign(this.$data.params, this.$options.data().params)
      },
      ...mapActions({
        getNodeTypeShowIndex: 'getNodeTypeShowIndex'
      })
    }
  }
</script>
