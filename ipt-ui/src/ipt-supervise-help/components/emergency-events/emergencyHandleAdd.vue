  ...
  export default {
    ...
    methods: {
      init() {
        ...
        TraceBackApi.ChainSynthesis.getChainSynthesisTableData({size: 0x7fffffff}).then(data => {
          data.data.forEach(item => {
            this.tempTranData[item.traceCode] = {
              tranCode: item.traceCode,
              relatedProduct: item.productName
            }
          })
        })
      ...
      create() {
        ...
        that.$validator.validateAll().then((result) => {
          if (result) {
            Api.EmergencyHandle.create(that.editedParams).then((data) => {
