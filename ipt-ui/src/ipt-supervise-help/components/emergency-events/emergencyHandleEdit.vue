            ...
            <div class="col-sm-7 input-list">
              <div v-for="(item, index) in editedParams.sourceTranCodes" :key="uuids[index]">
                <input type="text" v-model="editedParams.sourceTranCodes[index]"
                       ...
                       v-validate="'required|dupTranCode|availableTranCode'"
                ...
                <span v-show="errors.firstByRule('sourceTranCodes_' + uuids[index], 'dupTranCode')"
                      class="help is-danger">重复的溯源码</span>
                <span v-show="errors.firstByRule('sourceTranCodes_' + uuids[index], 'availableTranCode')"
  ...
  export default {
    ...
    watch: {
      singleData() {
        ...
        this.editedParams.sourceTranCodes = this.editedParams.details.map(item => {
          let sourceTranCode = item.sourceTranCode
          let tempTranDatum = this.tempTranData[sourceTranCode]
          this.uuids.push(window.uuidV4())
          this.relatedProducts.push(tempTranDatum ? tempTranDatum.relatedProduct : '')
          return sourceTranCode
        })
      }
    },
    data() {
    ...
    methods: {
      init() {
        ...
        Validator.extend('availableTranCode', {
      ...
      create() {
        ...
        that.$validator.validateAll().then((result) => {
          if (result) {
            Api.EmergencyHandle.update(that.editedParams.emergencyId, that.editedParams).then((data) => {
              if (data.status !== 200) {
                that.$alert('请求失败，请联系管理员！')
                return
              }
              ...
              that.getEmergencyHandleTableData(this.params)
            })
          }
        })
      },
