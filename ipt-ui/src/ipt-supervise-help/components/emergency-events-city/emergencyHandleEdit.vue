            ...
            <div class="col-sm-7 input-list">
              <div v-for="(item, index) in editedParams.sourceTranCodes" :key="uuids[index]">
                <input type="text" v-model="editedParams.sourceTranCodes[index]"
                       ...
                       v-validate="'required|dupTranCodeEdit|availableTranCodeEdit'"
                ...
                <span v-show="errors.firstByRule('sourceTranCodes_' + uuids[index], 'dupTranCodeEdit')"
                      class="help is-danger">重复的溯源码</span>
                <span v-show="errors.firstByRule('sourceTranCodes_' + uuids[index], 'availableTranCodeEdit')"
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.getEnterprises()
        Validator.extend('dupTranCodeEdit', {
        ...
        Validator.extend('availableTranCodeEdit', {
      ...
      create() {
        ...
        that.$validator.validateAll().then((result) => {
          if (result) {
            Api.EmergencyCityHandle.update(that.editedParams.crossCityEmergencyId, that.editedParams).then((data) => {
              if (data.status !== 200) {
