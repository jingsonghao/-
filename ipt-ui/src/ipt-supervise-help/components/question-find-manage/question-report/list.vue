<template>
  <div class="main">
    <ol class="breadcrumb">
      <li><a>监管辅助系统</a></li>
      <li><a>问题发现管理</a></li>
      <li class="active jsh-active">问题报告管理</li>
    </ol>
    <!-- [搜索框] -->
        ...
        <form class="form form-inline">
          ...
          <div class="form-group">
            ...
            <el-date-picker v-model="params.reportDateMin" style="width: 140px;"
                            type="date" placeholder="" format="yyyy-MM-dd" value-format="yyyy-MM-dd 00:00:00"></el-date-picker>
            <label>—</label>
            <el-date-picker v-model="params.reportDateMax" style="width: 140px;"
                            type="date" placeholder="" format="yyyy-MM-dd" value-format="yyyy-MM-dd 23:59:59"></el-date-picker>
          </div>
          <div class="form-group">
            <label>审核结果</label>
          ...
          <div class="form-group my-group">
            ...
            <button type="button" class="btn btn-reset1" data-toggle="modal" data-target="#js_add_modal"> 生成问题报告</button>
    ...
    <ipt-view id="js_show_modal" :questionReport="questionReport"></ipt-view>
    <ipt-edit id="js_edit_modal" :questionReport="questionReport" :params="params"></ipt-edit>
    <ipt-audit id="js_audit_modal" :questionReport="questionReport" :params="params"></ipt-audit>
    <ipt-auditNo id="js_auditNo_modal" :questionReport="questionReport" :params="params"></ipt-auditNo>
    ...
    <div class="shadow-boxs">
      <div class="col-lg-12">
        ...
        <ipt-table
          :tableData="questionReportTableData"
          :tableConfig.sync="questionReportConfig"
          :params="params">
          <!-- 插槽示例 -->
          ...
          <td slot="afterBody" slot-scope="scope">
            <a href="javascript:;" class="look-link" @click="showBtn(scope.columnData)" data-toggle="modal" data-target="#js_show_modal">查看</a>
            <!--   仅可编辑自身未通过   -->
            <a href="javascript:;" class="edit-link" @click="updateBtn(scope.columnData)" data-toggle="modal" data-target="#js_edit_modal" v-if="scope.columnData.auditStatus == 2 && scope.columnData.authorId === loginUser.userId">编辑</a>
            <!--   仅可审核未审核   -->
            <a href="javascript:;" class="add-link" @click="auditBtn(scope.columnData)" data-toggle="modal" data-target="#js_audit_modal" v-if="loginUser.roles[0].roleId == 1 && scope.columnData.auditStatus == 0">审核</a>
            <!--   仅可删除自身发布的未通过信息   -->
            <a href="javascript:;" class="delete-link" @click="deleteBtn(scope.columnData)" v-if="loginUser.roles[0].roleId == 1 || scope.columnData.auditStatus === 2 && scope.columnData.authorId === loginUser.userId">删除</a>
          </td>
        </ipt-table>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: mapGetters({
      questionReportTableData: 'questionReportTableData',
    ...
    methods: {
      ...
      showBtn(columnData) {
        Api.QuestionReport.findQuestionReport(columnData.reportId).then((data) => {
      ...
      approvalBtn(columnData) {
        this.$confirm('是否审核通过?', '提示', {
          ...
          Api.QuestionReport.approvalQuestionReport(columnData.reportId).then((data) => {
            this.$message({
              type: 'success',
              message: '审核成功!'
            })
      ...
      deleteBtn(columnData) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          Api.QuestionReport.deleteQuestionReport(columnData.reportId).then((data) => {
            this.$message({
              type: 'success',
      ...
      checkAll(event) {
        ...
        if (!event.currentTarget.checked) {
          ...
          self.checkboxList = self.questionReportTableData.data.map(item => item.reportId)
        }
      },
    ...
    watch: {
      checkboxList(curVal) {
        this.checkboxAll = curVal.length === this.questionReportConfig.display
      }
    }
  }
</script>
