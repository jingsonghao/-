<template>
  <div class="main">
    ...
    <div class="shadow-boxs">
      <ipt-table
        :tableData="personalBlacklistTableData"
  ...
  export default {
    ...
    data() {
      return {
        ...
        nodeTypes: [],
        tableConfig: {
          getApi: 'getPersonalBlacklistTableData',  // #必须# 数据的的请求Vuex的Action名
    ...
    computed: mapGetters({
      personalBlacklistTableData: 'personalBlacklistTableData'
    }),
    methods: {
      init() {
        /** Zhao Yun 2017/11/16 获取表格分页数据 */
        this.getPersonalBlacklistTableData(this.params)
      },
      search() {
        this.getPersonalBlacklistTableData(this.params)
      },
      ...
      checkAll(event) {
        ...
        if (!event.currentTarget.checked) {
          ...
          self.checkboxList = self.personalBlacklistTableData.data.map(item => item.enterpriseId)
        }
      },
    /** Zhao Yun 2017/11/16 获取请求数据的action */
      ...mapActions({
        getPersonalBlacklistTableData: 'getPersonalBlacklistTableData'
      })
  }
  }
</script>
