<template>
  <ipt-modal>
    <span slot="modalTitle">添加父部门</span>
    <template slot="modalBody">
      <form class="form-horizontal">
        <div class="form-group">
          <label class="col-sm-3 control-label">部门名称</label>
          <div class="col-sm-7">
            <input type="text" v-model="addParentParams.departmentName" class="form-control"
      ...
      saveAdd: function () {
        ...
        _this.$validator.validateAll().then((result) => {
          if (result && !_this.submitted) {
            ...
            Api.DepartmentManage.addParentDepartment(_this.addParentParams).then((data) => {
              _this.submitted = false
              _this.$alert(data.msg, '提示', {
                confirmButtonText: '确定',
                callback: action => {
                  if (data.status === 200) {
                    $('#departmentParentAdd div.modal-header > button[aria-label="Close"]').trigger('click')
