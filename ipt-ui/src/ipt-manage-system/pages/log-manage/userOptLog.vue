<template>
  <div class="main">
    <ol class="breadcrumb">
      <li><a>后台管理系统</a></li>
      <li><a>日志管理</a></li>
      <li class="active jsh-active">用户操作日志</li>
    </ol>
        ...
        <form class="form form-inline" id="query-form">
          <div class="form-group">
            ...
            <input type="text" class="form-control search-01" placeholder="请输入操作用户" style="width: 150px;"
                   v-model="userOptLogTableParams.username">
          </div>
          <div class="form-group">
            <label>操作类型</label>
            <select class="form-control search-01" v-model="userOptLogTableParams.opt">
          ...
          <div class="form-group">
            <label>方法类型</label>
            <select class="form-control search-01" v-model="userOptLogTableParams.method">
              <option value="">全部</option>
          ...
          <div class="form-group">
            <label>记录时间</label>
            <el-date-picker v-model="userOptLogTableParams.createTimeMin" style="width: 200px;"
                            type="datetime" placeholder="开始时间" format="yyyy-MM-dd HH:mm:ss"
                            value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
            <label>—</label>
            <el-date-picker v-model="userOptLogTableParams.createTimeMax" style="width: 200px;"
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control search-01" placeholder="请输入日志内容" style="width: 150px;"
                   v-model="userOptLogTableParams.msg">
          </div>
    ...
    <div class="shadow-boxs">
      <div class="col-lg-12">
        <ipt-table :tableData="userOptLogTableData"
                   :tableConfig.sync="userOptLogTableConfig"
                   :params="userOptLogTableParams">
        </ipt-table>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        userOptLogTableConfig: {
          getApi: 'getUserOptLogTableData',  // #必须# 数据的的请求Vuex的Action名
          title: "用户操作日志",             // [可选] 表格标题，默认不显示标题
    ...
    methods: {
      init: function () {
        this.initQueryList()
        this.getUserOptLogTableData(this.userOptLogTableParams)
      },
      ...
      reset: function () {
        Object.assign(this.$data.userOptLogTableParams, this.$options.data().userOptLogTableParams)
      },
      search: function () {
        this.getUserOptLogTableData(this.userOptLogTableParams)
      },
      ...mapActions({
        getUserOptLogTableData: 'getUserOptLogTableData'
      })
    },
    computed: mapGetters({
      userOptLogTableData: 'userOptLogTableData'
    })
  }
</script>
