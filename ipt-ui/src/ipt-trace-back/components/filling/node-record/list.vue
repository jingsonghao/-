<template>
  <div>
      ...
      <div class="r-search">
        <form class="form form-inline">
          <div class="form-group">
            ...
            <label>企业类型</label>
            <el-select v-model="params.enterpriseType" name="enterpriseType" filterable placeholder="请选择">
    ...
    <ipt-view id="js_show_modal" :enterpriseId="enterpriseId"></ipt-view>
    <ipt-edit id="js_edit_modal" :enterpriseId="enterpriseId" :params="params"></ipt-edit>
    <div class="shadow-boxs">
      <div class="col-lg-12">
        <!-- [表格] -->
        ...
        <ipt-table
          :tableData="nodeRecordList"
          :tableConfig.sync="nodeRecordConfig"
          :params="params">
          <!-- 插槽示例 -->
          ...
          <td slot="afterBody" slot-scope="scope">
            <a href="javascript:;" class="look-link" data-toggle="modal" data-target="#js_show_modal"
               @click="enterpriseId = scope.columnData.enterpriseId">查看</a>
            <a href="javascript:;" class="edit-link" data-toggle="modal" data-target="#js_edit_modal"
               @click="enterpriseId = scope.columnData.enterpriseId"
...
<script>
  ...
  export default {
    ...
    components: {
      IptTable, IptAdd, IptView, IptEdit
    },
    ...
    data() {
      return {
        ...
        nodeRecordConfig: {
          getApi: 'getNodeRecordList',  // #必须# 数据的的请求Vuex的Action名
    ...
    methods: {
      init() {
        ...
        this.getNodeRecordList(this.params)
      ...
      approvalBtn(columnData) {
        this.$confirm('是否审核通过?', '提示', {
          ...
          Api.NodeRecord.approval(columnData.enterpriseId).then((data) => {
            this.$message({
              type: 'success',
              message: '审核成功!'
            })
      ...
      deleteBtn(columnData) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          Api.NodeRecord.deleteOne(columnData.enterpriseId).then((data) => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            })
      ...
      checkAll(event) {
        ...
        if (!event.currentTarget.checked) {
          ...
          self.checkboxList = self.nodeRecordList.data.map(item => item.enterpriseId)
        }
      },
    ...
    watch: {
      checkboxList(curVal) {
        this.checkboxAll = curVal.length === this.nodeRecordConfig.display
      }
    }
  }
</script>
