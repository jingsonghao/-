<template>
  <ipt-modal>
    <!-- [modalTitle]标题插槽 -->
    <span slot="modalTitle">运行报告</span>
    <!-- [modalBody]内容插槽 -->
    <template slot="modalBody">
      <form class="form-horizontal">
        ...
        <div class="form-group">
          <label class="col-sm-3 control-label">请选择运行报告时间</label>
          <div class="col-sm-7">
            <el-date-picker v-model="runningReport.runningDateMin" style="width: 140px;"
                            type="date" placeholder="开始日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd 00:00:00"></el-date-picker>
            <label>—</label>
            <el-date-picker v-model="runningReport.runningDateMax" style="width: 140px;"
        ...
        <div class="form-group">
          ...
          <div class="col-sm-7">
            <input type="text" v-model="runningReport.reportName" name="reportName" class="form-control"
        ...
        <div class="form-group">
          ...
          <div class="col-sm-7">
            <vue-editor id="addNewsEditor" v-model="runningReport.content" useCustomImageHandler @imageAdded="handleImageAdded"></vue-editor>
            <!--<textarea rows="20" type="text" v-model="runningReport.content" name="content" class="form-control"
        ...
        <!--<div class="form-group">
          ...
          <div class="col-sm-7">
              <input type="text" v-model="runningReport.auditName" name="auditName" class="form-control">
          </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-7">
            <input type="text" v-model="runningReport.recipientName" name="recipientName" class="form-control" disabled>
          </div>
          <button class="btn btn-reset" type="button" data-toggle="modal" data-target="#js_user_modal" >选择人员</button>
          <input type="hidden" v-model="runningReport.recipientId">
        </div>
      </form>
    </template>
    <!-- [modalFooter]底部插槽 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      create() {
        ...
        that.$validator.validateAll().then((result) => {
          if (result) {
            Api.RunningReport.addRunningReport(that.runningReport).then((data) => {
              ...
              that.getRunningReportTableData(this.params)
            })
            return
          }
          this.$alert('Oh NO!')
    ...
    watch: {
      findUser() {
        this.runningReport.recipientName = this.findUser.name
        this.runningReport.recipientId = this.findUser.userId
      }
    },
    computed: mapGetters({
      findUser: 'findUser'
