<template>
  <li>
    <span>
      <b class="tree-icon" :class="'tree-icon' + iconIndex">
        <i>{{chainData.treadeDate}}</i>
        <br>
        <i>{{chainData.productName}} - {{chainData.weight}}KG</i>
      </b>
      <div class="explain row">
        <div class="col-xs-6 r">进厂环节：</div><div class="col-xs-6">{{chainData.nodeInName}}</div>
        <div class="col-xs-6 r">出厂环节：</div><div class="col-xs-6">{{chainData.nodeOutName}}</div>
        <div class="col-xs-6 r">商品名称：</div><div class="col-xs-6">{{chainData.productName}}</div>
        <div class="col-xs-6 r">交易时间：</div><div class="col-xs-6">{{chainData.treadeDate}}</div>
        <div class="col-xs-6 r">重 量：</div><div class="col-xs-6">{{chainData.weight}}KG</div>
        <div class="col-xs-4"></div>
        <!--<a class="col-xs-4 btn btn-xs" data-toggle="modal" data-target=".bs-example-modal-lg">查看更多>></a>-->
      </div>
    </span>
    <ul v-if="chainData.chainMiddleList.length != 0">
      <li></li>
      <ipt-chain-tree
                      :key="index"
                      v-for="(child, index) in chainData.chainMiddleList"
                      :chainData="child"
